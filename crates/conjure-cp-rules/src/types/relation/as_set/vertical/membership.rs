...
/// so membership delegates straight to whichever set representation `set_decl` ends up with.
#[register_rule("Base", 8650, [In])]
fn membership_relation_as_set(expr: &Expr, _: &SymbolTable) -> ApplicationResult {
    let Expr::In(metadata, member, collection) = expr else {
