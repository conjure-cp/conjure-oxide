...
/// Lower equality between a dense relation and a relation literal.
#[register_rule("ReprGeneral", 9500, [Eq])]
fn eq_relation_occurrence_literal(expr: &Expr, _: &SymbolTable) -> ApplicationResult {
    guard!(
    ...
    let Some(repr) = rel.get_repr_as::<RelationOccurrence>() else {
        return Err(RuleNotApplicable);
    };
    let Ok(constraint) = repr.equality_to_literal_expr(tuples) else {
        return Err(RuleNotApplicable);
    };
