...
fn equality_counts_mset_literal(expr: &Expr, _: &SymbolTable) -> ApplicationResult {
...
#[register_rule("ReprGeneral", 9500, [Eq])]
fn equality_counts_msets(expr: &Expr, _: &SymbolTable) -> ApplicationResult {
    guard!(let Expr::Eq(_, lhs, rhs) = expr &&
        ...
        let Some(lhs) = lhs.get_repr_as::<MSetCounts>() &&
        let Some(rhs) = rhs.get_repr_as::<MSetCounts>()
