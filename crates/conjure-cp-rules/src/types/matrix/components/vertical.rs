...
fn select_matrix_components(expr: &Expression, symtab: &SymbolTable) -> ApplicationResult {
    ...
    // [`select_matrix_components_after_comprehension_expansion`] on the materialised AC node
    // instead.
...
///
/// Comprehensions are Uniplate leaves, so [`select_matrix_components`] cannot see their bodies before
/// expansion. After a comprehension becomes an AC argument list, select once on that node so later
/// Root-level `select_matrix_components` attempts can stay O(declarations) rather than O(model).
#[register_rule("ReprMatrixComponents", 1990, [And, Or, Sum, Product])]
fn select_matrix_components_after_comprehension_expansion(
    expr: &Expression,
    _: &SymbolTable,
) -> ApplicationResult {
...
pub(crate) fn try_lower_const_unsafe_index_matrix_components(
    ...
    let components = re.ptr().get_repr::<MatrixComponents>()?;

    if indices.len() != components.index_domains.len() {
        return None;
    }
    ...
    for (domain, index) in components.index_domains.iter().zip(indices.iter()) {
    ...
    let mut elems = components.view_as_exprs(&view);
...
fn index_matrix_components_impl(expr: &Expression, symbols: &SymbolTable) -> ApplicationResult {
    guard!(
        ...
        let Some(components) = re.ptr().get_repr::<MatrixComponents>()
        else {
            return Err(RuleNotApplicable);
        }
    );
    let idx_doms = &components.index_domains;
        ...
        match idx_dom_gd {
            ...
            _ => {
                ...
                for idx_val in 0..dim_sz {
                    let orig_idx_val = components.index_flat_to_lit(di, idx_val).unwrap_or_bug();
...
fn slice_matrix_components(expr: &Expression, _: &SymbolTable) -> ApplicationResult {
    ...
    let permuted_elems: Vec<Expression> = components.view_as_exprs(&permuted_view);
...
fn flatten_matrix_components(expr: &Expression, _symbols: &SymbolTable) -> ApplicationResult {
    guard!(
...
fn expand_matrix_components_reference(
    expr: &Expression,
    _symbols: &SymbolTable,
) -> ApplicationResult {
        ...
        .map(|expr| {
                ...
                && let Some(components) = re.ptr().get_repr::<MatrixComponents>()
            {
                ...
                let elem_refs: Vec<Expression> =
                    components.flat_elem_refs().map(Expression::from).collect();
                let index_domains: Vec<DomainPtr> =
                    components.index_domains.iter().map(Into::into).collect();
                unflatten_matrix(&elem_refs, &index_domains, &components.strides)
            } else {
                expr
            }
        })
...
mod tests {
    ...
    #[test]
    fn select_matrix_components_skips_expression_walk_when_decls_already_initialised() {
        ...
        let rule = get_rule_by_name("select_matrix_components")
            .expect("select_matrix_components registered");
        let err = rule.apply(&root, &symbols).unwrap_err();
        assert!(matches!(err, ApplicationError::RuleNotApplicable));
    }

    #[test]
    fn select_matrix_components_after_comprehension_expansion_selects_emergent_refs() {
        ...
        let rule = get_rule_by_name("select_matrix_components_after_comprehension_expansion")
            .expect("select_matrix_components_after_comprehension_expansion registered");
