...
use conjure_cp::ast::{Expression, MaybeTypeable, ReturnType::Set, SymbolTable};
...
fn eq_to_subset_eq(expr: &Expression, _: &SymbolTable) -> ApplicationResult {
    match expr {
        Eq(_, a, b) => {
            if let Some(Set(_)) = a.as_ref().maybe_return_type() {
                if let Some(Set(_)) = b.as_ref().maybe_return_type() {
