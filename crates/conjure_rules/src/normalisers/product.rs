...
fn reorder_product(expr: &Expr, _: &SymbolTable) -> ApplicationResult {
    ...
    for expr in factors {
        match expr.category_of() {
            conjure_core::ast::categories::Category::Bottom => bottom_exprs.push(expr),
            conjure_core::ast::categories::Category::Constant => constant_exprs.push(expr),
            conjure_core::ast::categories::Category::Parameter => parameter_exprs.push(expr),
            conjure_core::ast::categories::Category::Quantified => quantified_exprs.push(expr),
            conjure_core::ast::categories::Category::Decision => decision_exprs.push(expr),
        }
    }
