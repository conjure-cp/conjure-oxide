...
pub(super) fn run_partial_evaluator(expr: &Expr, symtab: &SymbolTable) -> ApplicationResult {
    ...
    match expr.clone() {
        ...
        Expr::FlatSumLeq(_, _) => Err(RuleNotApplicable),
        Expr::FlatSumGeq(_,  _) => Err(RuleNotApplicable),
        Expr::FlatWatchedLiteral( _, _, _) => Err(RuleNotApplicable),
        Expr::FlatWeightedSumLeq( _, _) => Err(RuleNotApplicable),
        Expr::FlatWeightedSumGeq( _, _) => Err(RuleNotApplicable),
        ...
        Expr::MinionWInIntervalSet(_, _) => Err(RuleNotApplicable),
        Expr::MinionWInSet(_,  _) => Err(RuleNotApplicable),
