...
use uniplate::{derive::Uniplate, Biplate, Uniplate};
...
impl Comprehension {
    ...
    pub fn expand_ac(
        ...
        *generator_model.as_submodel_mut() = self.generator_submodel.clone();
        ...
        let generator_symtab_ptr2 = Rc::clone(&generator_symtab_ptr1);
        ...
        {
            let generator_symtab = (*generator_symtab_ptr1).borrow();

            assert!(
                Biplate::<Name>::universe_bi(&new_return_expr)
                    .iter()
                    .all(|x| generator_symtab.lookup_local(x).is_some()),
