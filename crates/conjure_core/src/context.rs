...
impl PartialEq for Context<'_> {
    fn eq(&self, other: &Self) -> bool {
        self.target_solver_family.read().unwrap().eq(&*other.target_solver_family.read().unwrap())
            && self.extra_rule_set_names.read().unwrap().eq(&*other.extra_rule_set_names.read().unwrap())
            && self.rules.read().unwrap().eq(&*other.rules.read().unwrap())
            && self.rule_sets.read().unwrap().eq(&*other.rule_sets.read().unwrap())
    }
}
...
impl<'a> Serialize for Context<'a> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error> where S: Serializer {
        ...
        let rules: Vec<String> = self.rules.read().unwrap().iter().map(|r| r.name.to_string()).collect();
        let rule_sets: Vec<String> = self.rule_sets.read().unwrap().iter().map(|rs| rs.name.to_string()).collect();
    ...
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error> where D: Deserializer<'de> {
            ...
            fn deserialize<D>(deserializer: D) -> Result<Self, D::Error> where D: Deserializer<'de> {
                ...
                impl<'de> Visitor<'de> for FieldVisitor {
                    ...
                    fn expecting(&self, formatter: &mut Formatter) -> std::fmt::Result {
                        formatter.write_str("target_solver_family, extra_rule_set_names, rules, rule_sets")
                    }
                    ...
                    fn visit_str<E>(self, value: &str) -> Result<Self::Value, E> where E: serde::de::Error {
                        match value {
                            ...
                            _ => Err(serde::de::Error::unknown_field(value, &["target_solver_family", "extra_rule_set_names", "rules", "rule_sets"])),
                        }
                    }
                }
        ...
        impl<'de> Visitor<'de> for ContextVisitor {
            ...
            fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error> where A: MapAccess<'de> {
                    ...
                    match key {
                        Field::TargetSolverFamily => {
                            if target_solver_family.is_some() {
                                return Err(serde::de::Error::duplicate_field("target_solver_family"));
                            }
                            target_solver_family = map.next_value()?;
                        }
                        Field::ExtraRuleSetNames => {
                            if extra_rule_set_names.is_some() {
                                return Err(serde::de::Error::duplicate_field("extra_rule_set_names"));
                            }
                        ...
                        Field::Rules => {
                            ...
                            let found_rules: Vec<&Rule> = rule_names.iter().filter_map(|name| get_rule_by_name(name)).collect();
                        ...
                        Field::RuleSets => {
                            ...
                            let found_rule_sets: Vec<&RuleSet> = rule_set_names.iter().filter_map(|name| get_rule_set_by_name(name)).collect();
                ...
                let extra_rule_set_names = extra_rule_set_names.ok_or_else(|| serde::de::Error::missing_field("extra_rule_set_names"))?;
                let rules = rules.ok_or_else(|| serde::de::Error::missing_field("rules"))?;
                let rule_sets = rule_sets.ok_or_else(|| serde::de::Error::missing_field("rule_sets"))?;
        ...
        const FIELDS: &[&str] = &["target_solver_family", "extra_rule_set_names", "rules", "rule_sets"];
