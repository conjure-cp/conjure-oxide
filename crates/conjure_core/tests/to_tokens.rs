...
pub fn test_enum() {
    ...
    assert_tokens_eq(&e, &quote! { TestEnum::E(vec![TestEnum::A.into(), TestEnum::B("World".into()).into()]) });
    assert_tokens_eq(&f1, &quote! { TestEnum::F(Some(Box::new(TestEnum::A.into()))) });
    assert_tokens_eq(&f2, &quote! { TestEnum::F(None) });
    assert_tokens_eq(&g, &quote! {
        TestEnum::G(
            vec![
