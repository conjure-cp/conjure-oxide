...
pub fn parse_essence_file(
        ...
        .output()
    {
        Ok(output) => output,
        Err(e) => {
            return Err(Box::new(ParseErrorCollection::new(vec![
                EssenceParseError::ConjurePrettyError(e.to_string()),
            ])));
        }
    };
    ...
    if !output.status.success() {
        ...
        return Err(Box::new(ParseErrorCollection::new(vec![
            EssenceParseError::ConjurePrettyError(stderr_string),
        ])));
    }
    ...
    let astjson = match String::from_utf8(output.stdout) {
        ...
        Err(e) => {
            return Err(Box::new(ParseErrorCollection::new(vec![
                EssenceParseError::ConjurePrettyError(format!(
    ...
    let parsed_model = model_from_json(&astjson, context)
        .map_err(|e| Box::new(ParseErrorCollection::new(vec![e.into()])))?;
    Ok(parsed_model)
}
