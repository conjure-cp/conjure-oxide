...
#[derivative(Clone(bound = "L: Clone + Eq + Hash, R: Clone + Eq + Hash"))]
#[derivative(Default(bound = "L: Default + Eq + Hash, R: Default + Eq + Hash"))]
#[derivative(PartialEq(bound = "L: Eq + Hash, R: Eq + Hash"))]
#[derivative(Eq(bound = "L: Eq + Hash, R: Eq + Hash"))]
    ...
    RAs: SerializeAs<R>,
{
        ...
        S: serde::Serializer,
    {
        let entries = source
            ...
            .map(|(left, right)| {
                (
                    SerializeAsWrap::<L, LAs>::new(left),
                    SerializeAsWrap::<R, RAs>::new(right),
                )
            })
            .collect::<Vec<_>>();
        entries.serialize(serializer)
    ...
    RAs: DeserializeAs<'de, R>,
{
    ...
    {
        let entries: Vec<(DeserializeAsWrap<L, LAs>, DeserializeAsWrap<R, RAs>)> =
            Vec::deserialize(deserializer)?;

        Ok(entries
            .into_iter()
            .map(|(left, right)| (left.into_inner(), right.into_inner()))
            .collect())
    }
}
