...
use super::convs::{SolutionVar, model_to_cp_sat, response_to_solution};
use super::ffi;
...
impl SolverAdaptor for OrToolsCpSat {
    ...
    fn solve(
        ...
        let cb = |response_proto: &[u8]| -> bool {
            ...
            let response = match CpSolverResponse::decode(response_proto) {
                Ok(r) => r,
                Err(e) => {
                    *rust_error.lock().unwrap() = Some(SolverError::Runtime(format!(
                        "Failed to decode OR-Tools response: {}",
                        e
                    )));
                    return false;
                }
        ...
        let final_response =
            CpSolverResponse::decode(response_bytes.as_slice()).map_err(|err| {
        ...
        let status = CpSolverStatus::try_from(final_response.status).map_err(|_| {
            SolverError::Runtime(format!(
                "Unknown OR-Tools solver status: {}",
        ...
        match status {
            CpSolverStatus::Optimal | CpSolverStatus::Feasible => Ok(SolveSuccess {
                stats,
            ...
            CpSolverStatus::Unknown => Err(SolverError::Runtime(
                if final_response.solution_info.is_empty() {
                    "OR-Tools returned UNKNOWN".to_owned()
                } else {
                    format!(
                        "OR-Tools returned UNKNOWN: {}",
                        final_response.solution_info
                    )
                },
            )),
    ...
    fn set_enumerate_all(&mut self, enumerate_all: bool) {
