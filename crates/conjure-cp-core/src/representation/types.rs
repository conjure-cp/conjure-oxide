...
pub trait ReprRule: Send + Sync {
    ...
    fn init_for(decl: &mut DeclarationPtr) -> ReprResult {
        ...
        // Named domain lettings are represented by an unresolved reference in a
        // declaration.  Representation initialisers need the domain behind that
        // reference; preserve genuinely unresolved domains for rules which can
