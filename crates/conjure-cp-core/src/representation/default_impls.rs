...
/// Returns the number of values in a representation-variable domain.
/// Domains whose size is unknown or does not fit in `usize` rank as least compact.
pub fn domain_size(domain: &DomainPtr) -> usize {
    domain
        .length()
        .ok()
