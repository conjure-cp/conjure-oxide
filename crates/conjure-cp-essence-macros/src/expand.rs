...
use conjure_cp_essence_parser::{
    ...
    util::{get_tree_fragment, query_toplevel},
};
use polyquine::Quine;
...
pub fn expand_expr(essence: &TokenTree) -> Result<TokenStream> {
    ...
    let (tree, source_code) = get_tree_fragment(&src)
        .ok_or(Error::new(essence.span(), "Could not parse Essence AST"))?;
...
pub fn expand_expr_vec(tt: &TokenTree) -> Result<TokenStream> {
    ...
    let (tree, source_code) =
        get_tree_fragment(&src).ok_or(Error::new(tt.span(), "Could not parse Essence AST"))?;
    let root = tree.root_node();
