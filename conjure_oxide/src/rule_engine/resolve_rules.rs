...
fn resolve_rule_sets<'a>(
    ...
    for rule_set_name in rule_set_names {
        let new_dependencies = resolve_dependencies(rule_set_name.as_str())?;
        rs_set.extend(new_dependencies);
    }

    Ok(rs_set.into_iter().map(|rs| rs).collect())
}
...
pub fn get_rules_vec<'a>(rule_priorities: &HashMap<&'a Rule<'a>, u8>) -> Vec<&'a Rule<'a>> {
    let mut rules: Vec<&'a Rule<'a>> = rule_priorities.keys().map(|rule| *rule).collect();
