...
- `make test-accept` / `ACCEPT=true` also rewrites `expected-time` (and other timing fields) from the current run. Do **not** blindly commit those timing updates: runtimes are noisy across machines and load, and tiny time-only diffs create pointless churn. Prefer leaving small timing wobbles uncommitted; only land timing changes when they reflect a real, intentional shift (for example a new slow test, or a deliberate budget change). Use `make test-accept-with-max-times` when recording budgets over several runs: each run keeps the max of the current budget and the observed time, so a fast fluke does not lower it. To discard files whose only local edits are timing fields, run `./tools/discard-config-time-changes.sh` (see `--help` for `--dry-run`). Files with any non-time edits are left alone.
