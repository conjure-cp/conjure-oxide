#!/usr/bin/env python3
"""
Analyse Multiple Variables experiment results and produce plots.
...
Linear scaling varies N (number of top-level decision variables) with
fixed shape M x K.
"""
...
def plot_e2e(df):
    ...
    for ax, kind in zip(axes[0], kinds):
        ...
        sns.despine(ax=ax)
    _set_suptitle(fig, "Multiple Variables: End-to-End Time")
    fig.tight_layout()
...
def plot_translation_time(df):
    ...
    for ax, kind in zip(axes[0], kinds):
        ...
        sns.despine(ax=ax)
    _set_suptitle(fig, "Multiple Variables: Solution Translation Time")
    fig.tight_layout()
...
def plot_rewrite_time(df):
    ...
    for ax, kind in zip(axes[0], kinds):
        ...
        ax.grid(True, alpha=0.3, which="both")
        sns.despine(ax=ax)
    _set_suptitle(fig, "Multiple Variables: Rewrite Time")
    fig.tight_layout()
...
def plot_num_solutions(df):
    ...
    for ax, kind in zip(axes[0], kinds):
        ...
        sns.despine(ax=ax)
    _set_suptitle(fig, "Multiple Variables: Problem Scale")
    fig.tight_layout()
...
def plot_breakdown(df):
    ...
    for kind in df["Kind"].unique():
        ...
        g.set(yscale="log")
        g.set_axis_labels("Number of variables N", "Time (s, log scale)")
        if _TITLE_FP is not None:
            g.figure.suptitle(
                f"Multiple Variables ({kind}): Phase Breakdown",
...
def plot_e2e_vs_solutions(df):
    ...
    for ax, kind in zip(axes[0], kinds):
        ...
        ax.grid(True, alpha=0.3, which="both")
        sns.despine(ax=ax)
    _set_suptitle(fig, "Multiple Variables: E2E Time vs Solutions")
    fig.tight_layout()
...
def plot_rewrite_vs_solutions(df):
    ...
    for ax, kind in zip(axes[0], kinds):
        ...
        ax.grid(True, alpha=0.3, which="both")
        sns.despine(ax=ax)
    _set_suptitle(fig, "Multiple Variables: Rewrite Time vs Solutions")
    fig.tight_layout()
...
def plot_translation_vs_solutions(df):
    ...
    for ax, kind in zip(axes[0], kinds):
        ...
        ax.grid(True, alpha=0.3, which="both")
        sns.despine(ax=ax)
    _set_suptitle(fig, "Multiple Variables: Translation Time vs Solutions")
    fig.tight_layout()
