#!/usr/bin/env python3
"""
Benchmark linearly-scaled matrix and record models.

Keeps structure fixed, scales the number of top-level decision variables.
Each variable has the same shape -- N * M leaf ints in int(1..K).

Matrix: find x1: matrix indexed by [int(1..M)] of int(1..K)  ...
Record: find x1: record {f1: int(1..K), ..., fM: int(1..K)}  ...
"""
...
M = 2       # entries per variable (matrix length / record fields)
K = 2       # leaf domain: int(1..K)
MAX_N = 8   # max number of decision variables
...
def gen_matrix_model(n: int, m: int, k: int) -> str:
    lines = [f"find x{i}: matrix indexed by [int(1..{m})] of int(1..{k})"
             for i in range(1, n + 1)]
    return "\n".join(lines) + "\n"


def gen_record_model(n: int, m: int, k: int) -> str:
    fields = ", ".join(f"f{j}: int(1..{k})" for j in range(1, m + 1))
    record = f"record {{{fields}}}"
    lines = [f"find x{i}: {record}" for i in range(1, n + 1)]
    return "\n".join(lines) + "\n"


# -- oxide runner ------------------------------------------------------------

def run_one_oxide(kind: str, n: int, run: int, m: int, k: int, oxide_bin: str) -> dict:
    model_text = gen_matrix_model(n, m, k) if kind == "matrix" else gen_record_model(n, m, k)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"[oxide {kind} N={n} run={run}] {num_solutions} solutions",
              file=sys.stderr)
        return {
            "system": "oxide", "kind": kind, "M": m, "K": k,
            "N": n, "run": run,
...
def run_one_conjure(kind: str, n: int, run: int, m: int, k: int) -> dict:
    model_text = gen_matrix_model(n, m, k) if kind == "matrix" else gen_record_model(n, m, k)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"[conjure {kind} N={n} run={run}] {stats['num_solutions']} solutions",
              file=sys.stderr)
        return {
            "system": "conjure", "kind": kind, "M": m, "K": k,
            "N": n, "run": run,
...
def _error_row(system: str, kind: str, n: int, run: int) -> dict:
    return {"system": system, "kind": kind, "M": M, "K": K,
...
def main() -> None:
        ...
        with ThreadPoolExecutor(max_workers=n_threads) as pool:
            ...
            for kind, n in jobs:
                for run in range(1, n_runs + 1):
                    f = pool.submit(run_one_oxide, kind, n, run, M, K, oxide_bin)
                    futs[f] = (kind, n, run)
    ...
    if do_conjure:
        with ThreadPoolExecutor(max_workers=n_threads) as pool:
            ...
            for kind, n in jobs:
                for run in range(1, n_runs + 1):
                    f = pool.submit(run_one_conjure, kind, n, run, M, K)
                    futs[f] = (kind, n, run)
            for f in as_completed(futs):
                kind, n, run = futs[f]
                try:
    ...
    fieldnames = ["system", "kind", "M", "K", "N", "run", "num_solutions",
