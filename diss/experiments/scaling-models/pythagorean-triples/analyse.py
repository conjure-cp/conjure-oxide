#!/usr/bin/env python3
"""
Analyse Pythagorean Triples experiment results and produce plots.
...
def plot_e2e(df):
    ...
    _set_title(ax, "Pythagorean Triples: End-to-End Time")
...
def plot_translation_time(df):
    ...
    _set_title(ax, "Pythagorean Triples: Solution Translation Time")
    ax.legend(prop=_LEGEND_PROPS)
...
def plot_rewrite_time(df):
    ...
    _set_title(ax, "Pythagorean Triples: Rewrite Time")
    ax.legend(prop=_LEGEND_PROPS)
    ax.grid(True, alpha=0.3, which="both")
    sns.despine(ax=ax)
...
def plot_num_solutions(df):
    ...
    _set_title(ax, "Pythagorean Triples: Problem Scale")
...
def plot_breakdown(df):
    ...
    g.set(yscale="log")
    g.set_axis_labels("Partition size N", "Time (s, log scale)")
    g.figure.suptitle(
        "Pythagorean Triples: Phase Breakdown",
