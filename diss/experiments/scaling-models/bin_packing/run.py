#!/usr/bin/env python3
"""
Scale the bin packing model (N items, fixed capacity) from MIN_N to MAX_N,
comparing conjure-oxide vs old Conjure. Writes results.csv for plotting.

Items are generated deterministically from N:
    weight_i = (i * 7 % cap) + 1
    colour_i = (i % ceil(N/2)) + 1
"""
...
def generate_items(n: int, cap: int) -> list[tuple[int, int]]:
    """Deterministic (weight, colour) tuples. Some colours repeat so the
    same-colour-can't-share constraint is non-trivial."""
    num_colours = max(1, math.ceil(n / 2))
    return [((i * 7 % cap) + 1, (i % num_colours) + 1) for i in range(1, n + 1)]


def items_to_essence(items: list[tuple[int, int]]) -> str:
...
def generate_params(min_n: int, max_n: int) -> list[tuple[int, Path]]:
    ...
    for n in range(min_n, max_n + 1):
        items = generate_items(n, CAP)
        p = PARAMS_DIR / f"n_{n}.param"
        p.write_text(
            f"language Essence 1.0\n\n"
            f"letting n be {n}\n"
            f"letting cap be {CAP}\n\n"
            f"letting items be\n"
            f"    {items_to_essence(items)}\n"
        )
        jobs.append((n, p))
    return jobs
...
def run_one_oxide(n: int, run: int, param_path: Path, oxide_bin: str,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        return {
            "system": "oxide", "N": n, "run": run, "cap": CAP,
...
def run_one_conjure(n: int, run: int, param_path: Path,
    ...
    return {
        "system": "conjure", "N": n, "run": run, "cap": CAP,
...
def _error_row(system: str, n: int, run: int) -> dict:
    return {"system": system, "N": n, "run": run, "cap": CAP, "num_solutions": -1,
...
def main() -> None:
    ...
    fieldnames = ["system", "N", "run", "cap", "num_solutions", "rewrite_time_ms",
