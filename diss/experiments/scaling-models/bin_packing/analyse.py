#!/usr/bin/env python3
"""
Analyse Bin Packing experiment results and produce plots.
...
Reads results.csv and generates:
  ...
  5. Phase breakdown per N, faceted by system (log scale)

The CSV has a 'cap' column (bin capacity), currently fixed at 10.
"""
...
def plot_e2e(df):
    ...
    _set_title(ax, "Bin Packing: End-to-End Time")
...
# -- plot 2: solution translation time (log) --------------------------------
...
def plot_translation_time(df):
    ...
    _set_title(ax, "Bin Packing: Solution Translation Time")
    ax.legend(prop=_LEGEND_PROPS)
...
def plot_rewrite_time(df):
    ...
    _set_title(ax, "Bin Packing: Rewrite Time")
    ax.legend(prop=_LEGEND_PROPS)
    ax.grid(True, alpha=0.3, which="both")
    sns.despine(ax=ax)
...
def plot_num_solutions(df):
    ...
    _set_title(ax, "Bin Packing: Problem Scale")
...
def plot_breakdown(df):
    ...
    g.set(yscale="log")
    g.set_axis_labels("Number of items N", "Time (s, log scale)")
    g.figure.suptitle(
        "Bin Packing: Phase Breakdown",
