...
RUSTDOCFLAGS="-Zunstable-options --show-type-layout --markdown-no-toc" cargo +nightly doc --no-deps --all-features -p conjure_oxide -p conjure_rule_macros -p conjure_core -p minion-sys -p enum_compatability_macro -p conjure_essence_parser -p conjure_essence_macros $@
