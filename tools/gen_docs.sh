...
RUSTDOCFLAGS="-Zunstable-options --show-type-layout --markdown-no-toc" cargo +nightly doc --no-deps --all-features -p conjure_oxide -p conjure-cp-rule-macros -p conjure-cp-core -p minion-sys -p conjure-cp-enum-compatibility-macro -p conjure-cp-essence-parser -p conjure-cp-essence-macros -p conjure-cp $@
