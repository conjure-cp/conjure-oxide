...
### Partition Operators
There are 5 operators for [Partitions](tbc), three return sets (`Parts`, `Party`, `Participants`) and two return booleans (`Together`, `Apart`). The current implementations of these may be improved upon, and have just been added as a part of supporting the Partition type.

### Sequence Operators
There are 2 operators for [Sequences](tbc) (`Subsequence`, `Substring`), and booth return booleans. They are both of the form `s sub.. t`, where `s` must occur in `t`. You can conclude that either a `Subsequence` or `Substring` is definitively false if `s` is strictly larger than `t`, but as of writing this documentation there is no mechanism to 'restrict' the domain of a boolean to being strictly true or false.
