...
Syntactic errors result in an invalid **Concrete Syntax Tree (CST)** generated by the tree-sitter parser ([Tree-sitter](https://tree-sitter.github.io/tree-sitter/)) when parsing Essence code. An invalid CST may contain:

- **`ERROR` nodes**  
  Inserted when the parser fails to interpret a portion of the code according to the [grammar](https://github.com/conjure-cp/conjure-oxide/blob/main/crates/tree-sitter-essence/grammar.js).
