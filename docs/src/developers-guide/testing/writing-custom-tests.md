...
The custom tests are run automatically when `cargo nextest run` is run. You can run just the custom tests with `cargo nextest run -E 'test(custom)'` and a specific test with `cargo nextest run -E 'test(custom_<test_path>)'`. To run one of several tests, use `or`, for example `cargo nextest run -E 'test(custom_foo) or test(custom_bar)'`. To overwrite the expected output/error with the actual output/error (and refresh `expected-time` in `config.toml`), run the tests with `ACCEPT=true`. Integration tests keep the same timing metadata in `stats.toml` instead. See `test-suite/README.md` and `CONTRIBUTING.md` for accept modes and guidance on avoiding timing-only churn in `config.toml` / `stats.toml`; `./tools/discard-config-time-changes.sh` can restore files from HEAD when their only local edits are `*-time` fields.
