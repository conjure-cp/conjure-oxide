...
This was discussedin Nov 2025. The team decided to keep the generated `src/` directory because:
- It matches the standard `tree-sitter` repository layout and ecosystem expectations.
- It keeps the parser compatible with the tree-sitter CLI and neovim.
- Generating parser artifacts during normal Conjure Oxide builds/runs would introduce a Node.js dependency into the Rust build path.

These files may cause merge conflicts but they are easily resolved by simply running `tree-sitter generate` again and committing that result. The only file that ever has to be manually changed or reviewed is `grammar.js`.
