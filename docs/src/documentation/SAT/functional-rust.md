...
This also allows for the use of the `?` operator[^5], which means that errors can be progogated up the call stack lazily. So, something that, in an entirely imperative language may look like the following pseudocode:
...
public void fnReturnsError(a,b) {
    ...Some Code...
    // might throw error1
    int foo1 = maybeReturnsMyError1("Both values are negative");
    ...Some More Code...
    // might throw error2
    int foo2 = maybeReturnsMyError2("First value is negative");
    ...Wow, when will this code end...
    // might throw error3
    int foo3 = maybeReturnsMyError3("Second value is negative");
}
```

Alternatively, consider a lower-level language like C, which is far more comparable to Rust in its uses and applications, where there are no 'error types' and errors behave like either enums or just integral exit codes. This means that, like rust, an error is not an interrupt, but a value that is being returned. However, because these languages are more imperative than rust, they require these exit codes to be set and returned at the end. This also means that the function calls must be done on some top-level data structure, and that global structure needs to be accessed at the end rather than being able to simply access the top-level data structure through the returned value. 

In this case, Rust offers two advantages:
...
In a C-like language, this would look like this:

```C
```

Would look like this in rust-like functional (pseudo)code:
...
1. Make the greatest effort to treat Rust as a functional language when programming in this (and indeed any) codebase. Not only does this lead to cleaner, more concise and (arguably) more readable code. 
