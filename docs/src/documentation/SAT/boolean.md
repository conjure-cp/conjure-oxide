...
The most basic form of a constraint is a boolean expression. SAT solvers work with boolean variables, but require input in CNF (conjunctive normal form). This means that any boolean expression must be converted to a series of clauses composed entirely of atoms (true/false values or variables). Here's an example:
