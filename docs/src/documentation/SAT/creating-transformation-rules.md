...
#### Domain propagation & returning the result

For negation, calculating the new domain is very simple:
For
$` x \in [a, b] `$ The new range is $` -x \in [-b, -a]`$. So here the domain propagation is calculated within the output.
